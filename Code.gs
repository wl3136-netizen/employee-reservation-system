...
const CONFIG={TIME_ZONE:'Asia/Seoul',SESSION_TTL_SECONDS:21600,SCHEDULE:{'2026-09-30':['티에스이 2사업장','지엠테스트'],'2026-10-01':['티에스이 2사업장','지엠테스트'],'2026-10-02':['티에스이 2사업장','메가센'],'2026-10-06':['메가터치','메가터치(티에스이)'],'2026-10-07':['메가터치','메가터치(티에스이)'],'2026-10-08':['메가센(티에스이)'],'2026-10-12':['티에스이 3사업장','메가센'],'2026-10-13':['티에스이 3사업장'],'2026-10-15':['타이거일렉 사업장'],'2026-10-16':['티에스이 3사업장','타이거일렉 사업장'],'2026-10-19':['엘디티(티에스이)'],'2026-10-20':['엘디티(티에스이)'],'2026-10-21':['티에스이 3사업장'],'2026-10-22':['티에스이 3사업장'],'2026-10-23':['티에스이 3사업장'],'2026-10-26':['엘디티'],'2026-10-27':['엘디티'],'2026-10-28':['엘디티(티에스이)'],'2026-10-29':['티에스이 3사업장']},SHEETS:{USERS:'Users',DEPARTMENTS:'Departments',SLOTS:'AvailableSlots',RESERVATIONS:'Reservations',MASTERS:'Masters'}};
...
function isBookableDate_(date){date=validDate_(normalizeDate_(date));const day=new Date(`${date}T00:00:00+09:00`).getDay();return day!==0&&day!==6}
function isScheduledDate_(date){return isBookableDate_(date)&&scheduleLocations_(date).length>0}
...
function isSlotOpenFor_(s,company){if(!bool_(s.enabled)||!isBookableDate_(s.date))return false;const location=String(s.location||'').trim();return!!location&&(company===null||locationCompany_(location)===company)}
...
function saveAvailability_(req){auth_(req,true);const x=req.availability||{},date=validDate_(normalizeDate_(x.date)),start=validTime_(normalizeTime_(x.startTime)),end=validTime_(normalizeTime_(x.endTime)),locations=scheduleLocations_(date),location=requiredText_(x.location||(locations.length===1?locations[0]:''),'장소');if(!isBookableDate_(date))throw new AppError('주말에는 일정을 등록할 수 없습니다.');if(start>=end)throw new AppError('종료 시간은 시작 시간보다 늦어야 합니다.');const id=x.slotId||uuid_();upsert_(CONFIG.SHEETS.SLOTS,'slotId',id,[id,date,start,end,!!x.enabled,location]);return{slotId:id}}
function slotTimeRows_(){const seen={},times=[];rows_(CONFIG.SHEETS.SLOTS).forEach(s=>{const start=normalizeTime_(s.startTime),end=normalizeTime_(s.endTime),key=`${start}|${end}`;if(!seen[key]&&/^\d{2}:\d{2}$/.test(start)&&/^\d{2}:\d{2}$/.test(end)){seen[key]=true;times.push([start,end])}});return times.sort((x,y)=>x[0].localeCompare(y[0]))}
function addScheduleDay_(req){auth_(req,true);const x=req.day||{},date=validDate_(normalizeDate_(x.date)),location=requiredText_(x.location,'장소');if(!isBookableDate_(date))throw new AppError('주말에는 일정을 등록할 수 없습니다.');const times=slotTimeRows_();if(!times.length)throw new AppError('등록된 운영 시간이 없습니다. 일정을 하나 먼저 추가해 주세요.');const existing=new Set(rows_(CONFIG.SHEETS.SLOTS).filter(s=>normalizeDate_(s.date)===date&&String(s.location||'').trim()===location).map(s=>`${normalizeTime_(s.startTime)}|${normalizeTime_(s.endTime)}`));const rows=times.filter(([start,end])=>!existing.has(`${start}|${end}`)).map(([start,end])=>[uuid_(),date,start,end,true,location]);if(!rows.length)throw new AppError('이미 등록된 날짜와 장소입니다.');sheet_(CONFIG.SHEETS.SLOTS).getRange(sheet_(CONFIG.SHEETS.SLOTS).getLastRow()+1,1,rows.length,HEADERS.AvailableSlots.length).setValues(rows);return{date,location,company:locationCompany_(location),slots:rows.length}}
...
function resetSystemFromImportedSheets(){
  ...
  const dates=Object.keys(CONFIG.SCHEDULE).filter(isScheduledDate_);
...
function scheduleSlotRows_(timeRows){const rows=[];Object.keys(CONFIG.SCHEDULE).filter(isScheduledDate_).sort().forEach(date=>scheduleLocations_(date).forEach(location=>timeRows.forEach(([start,end])=>rows.push([uuid_(),date,start,end,true,location]))));return rows}

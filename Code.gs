...
function isSlotOpenFor_(s,company){if(!bool_(s.enabled)||!isBookableDate_(s.date))return false;const location=String(s.location||'').trim();return scheduleLocations_(normalizeDate_(s.date)).includes(location)&&locationCompany_(location)===company}
...
function getAvailableDates_(req){const a=auth_(req,false),prefix=validMonth_(req);return[...new Set(openSlotsFor_(a.user.company).filter(s=>s.available&&s.date.startsWith(prefix)).map(s=>s.date))].sort()}
function getAvailableSlots_(req){const a=auth_(req,false),date=validDate_(normalizeDate_(req.date));return openSlotsFor_(a.user.company).filter(s=>s.date===date)}
function getCalendarMonth_(req){const a=auth_(req,false),prefix=validMonth_(req);return openSlotsFor_(a.user.company).filter(s=>s.date.startsWith(prefix))}
function getCalendarAll_(req){const a=auth_(req,false);return openSlotsFor_(a.user.company)}
function createReservation_(req){const a=auth_(req,false),slotId=String(req.slotId||'');if(!slotId)throw new AppError('예약 시간을 선택해 주세요.');const lock=LockService.getScriptLock();try{lock.waitLock(10000)}catch(e){throw new AppError('예약 요청이 많습니다. 잠시 후 다시 시도해 주세요.')}try{const slot=rows_(CONFIG.SHEETS.SLOTS).find(s=>s.slotId===slotId);if(!slot||!bool_(slot.enabled))throw new AppError('예약할 수 없는 시간입니다.');const date=validDate_(normalizeDate_(slot.date)),start=validTime_(normalizeTime_(slot.startTime)),end=validTime_(normalizeTime_(slot.endTime)),location=String(slot.location||'').trim();if(!isBookableDate_(date)||!scheduleLocations_(date).includes(location))throw new AppError('예약할 수 없는 날짜입니다.');if(locationCompany_(location)!==a.user.company)throw new AppError(`${location}은(는) ${locationCompany_(location)} 소속만 예약할 수 있습니다.`);if(start>=end)throw new AppError('예약 시간 설정이 올바르지 않습니다.');if(isPast_(date,start))throw new AppError('이미 지난 시간은 예약할 수 없습니다.');const reservations=rows_(CONFIG.SHEETS.RESERVATIONS);if(reservations.some(r=>r.status==='ACTIVE'&&r.userId===a.user.userId))throw new AppError('예약은 1인당 1개만 가능합니다. 기존 예약을 취소한 후 다시 예약해 주세요.');if(reservations.some(r=>r.status==='ACTIVE'&&slotKey_(normalizeDate_(r.date),normalizeTime_(r.startTime),normalizeTime_(r.endTime),r.location)===slotKey_(date,start,end,location)))throw new AppError('방금 다른 사용자가 예약한 시간입니다. 다른 시간을 선택해 주세요.');const reservationId=uuid_();sheet_(CONFIG.SHEETS.RESERVATIONS).appendRow([reservationId,date,start,end,a.user.userId,a.user.company,a.user.name,a.user.departmentId,'ACTIVE',new Date(),location]);return{reservationId}}finally{lock.releaseLock()}}

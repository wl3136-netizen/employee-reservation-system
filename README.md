...
- 예약 가능한 날짜와 사업장은 `Code.gs`의 `CONFIG.SCHEDULE`에서 관리합니다. 목록에 없는 날짜와 주말은 예약할 수 없습니다.
...
- `CONFIG.SCHEDULE`을 바꾼 뒤에는 Apps Script에서 `rebuildSlotsFromSchedule()`를 한 번 실행해 `AvailableSlots`를 다시 만듭니다. 사용자와 예약 내역은 유지됩니다.

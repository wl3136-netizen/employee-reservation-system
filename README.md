...
- 사업부: `DEV` 기술개발, `MFG` 제조
- 일반 사용자: `100001` 김테스트 / 기술개발
- 마스터 겸 사용자: `900001` 관리자 / 기술개발
- 예약 슬롯: 배포 시점 이후 날짜의 예시 일정

운영 전에는 테스트 이름·사번과 슬롯을 실제 값으로 교체하세요. `Masters`에만 있고 `Users`에 없는 사번은 로그인할 수 없습니다.
...
4. 일반 사용자 `100001`로 로그인하여 달력의 예약 가능 날짜와 시간을 확인합니다.
...
7. 마스터 `900001`로 사용자·사업부·일정 등록/수정 및 전체 예약 취소를 확인합니다.
...
```json
{"action":"login","employeeId":"100001"}
```

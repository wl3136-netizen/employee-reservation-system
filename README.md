...
- 예약 가능한 날짜와 사업장은 `AvailableSlots` 시트가 기준입니다. 관리자 화면의 `예약 일정 관리`에서 추가·수정할 수 있고, 주말은 등록할 수 없습니다.
- `날짜 추가(전체 시간)` 버튼은 날짜와 장소를 받아 등록된 운영 시간 전체에 슬롯을 한 번에 만듭니다. 같은 날짜·장소에 이미 있는 시간은 건너뜁니다.
- `Code.gs`의 `CONFIG.SCHEDULE`은 `rebuildSlotsFromSchedule()`로 슬롯을 한 번에 다시 만들 때 쓰는 기준표입니다. 평소 운영은 관리자 화면에서 합니다.
- 사업장명에 괄호가 있으면 괄호 안 회사 소속만, 없으면 사업장명 첫 단어의 회사 소속만 예약할 수 있습니다. 예: `엘디티(티에스이)` → 티에스이, `엘디티` → 엘디티, `티에스이 3사업장` → 티에스이
- 사용자는 달력에서 자기 회사가 예약할 수 있는 사업장 일정만 봅니다. 같은 날 사업장이 여러 곳이면 사업장별로 슬롯이 따로 만들어집니다.
- `rebuildSlotsFromSchedule()`는 `AvailableSlots`를 지우고 `CONFIG.SCHEDULE` 기준으로 다시 만듭니다. 관리자 화면에서 추가한 날짜도 함께 지워지므로, 전체를 다시 맞출 때만 실행하세요. 사용자와 예약 내역은 유지됩니다.
